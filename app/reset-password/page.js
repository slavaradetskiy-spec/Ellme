...
export default function ResetPasswordPage() {
  ...
  const [status, setStatus] = useState('loading') // loading | ready | success | expired
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'PASSWORD_RECOVERY') {
        ...
        // Some versions use SIGNED_IN for recovery
        setStatus('ready')
      }
    })
  ...
  const inputStyle = { width: '100%', padding: '16px 48px 16px 18px', borderRadius: 16, border: `1.5px solid ${C.tileBorder}`, fontSize: 15, fontFamily: "'Plus Jakarta Sans',sans-serif", outline: 'none', boxSizing: 'border-box', background: C.surface, color: C.text, marginBottom: 12 }

  const EyeBtn = ({ show, toggle }) => (
    <button type="button" onClick={toggle} style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: C.muted, display: 'flex', padding: 2 }}>
      ...
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        {status === 'loading' && (
          <div style={{ textAlign: 'center', padding: 40 }}>
            <p style={{ color: C.muted, fontSize: 14 }}>Проверяем ссылку...</p>
          </div>
        )}
        ...
        {status === 'expired' && (
          <div style={{ textAlign: 'center', padding: 20 }}>
            ...
            <p style={{ color: C.soft, fontSize: 14, lineHeight: 1.6, marginBottom: 24 }}>
              Ссылка недействительна или истекла. Запросите восстановление пароля заново.
            </p>
            <a href="/" style={{ display: 'inline-block', padding: '14px 32px', borderRadius: 16, background: C.accent, color: '#fff', fontSize: 15, fontWeight: 600, textDecoration: 'none' }}>
              На главную
            </a>
          </div>
        )}
        ...
        {status === 'ready' && (
          <>
            ...
            <div style={{ position: 'relative' }}>
              <input type={showPw ? 'text' : 'password'} value={pw} onChange={e => setPw(e.target.value)} placeholder="Новый пароль (мин. 6 символов)" style={inputStyle}
            ...
            <div style={{ position: 'relative' }}>
              <input type={showPw2 ? 'text' : 'password'} value={pw2} onChange={e => setPw2(e.target.value)} placeholder="Повторите пароль" style={inputStyle}
                onFocus={e => e.target.style.borderColor = C.accent} onBlur={e => e.target.style.borderColor = C.tileBorder}
